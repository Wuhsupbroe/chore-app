...
// ═══════════════════════════════════════════════════════
import { auth, db, state, showScreen, openModal, closeModal, genCode, hexToRgb, todayStr, renderEmojiGrid, KID_EMOJIS, CHORE_EMOJIS, THEME_COLORS,
  signInWithEmailAndPassword, createUserWithEmailAndPassword, signInWithPopup, GoogleAuthProvider, onAuthStateChanged, signOut,
  doc, getDoc, setDoc, addDoc, updateDoc, deleteDoc, collection, query, where, getDocs, onSnapshot, serverTimestamp } from "./app-core.js";
...
function startReminderScheduler() { if (_reminderStarted) return; _reminderStarted = true; checkReminders(); setInterval(checkReminders, 60000); }
function checkReminders() {
  ...
  if (h===17 && m===0 && state.kidId && state.familyIdKid) {
    ...
    if (!localStorage.getItem(key)) {
      ...
      if (incomplete.length) { sendNotification("ChoreQuest ⭐", `You still have ${incomplete.length} chore${incomplete.length>1?"s":""} left today!`); localStorage.setItem(key,"1"); }
    }
  }
  if (h===21 && m===0 && state.parentUser && state.familyId) {
    const key = `parentSum-${todayStr()}`;
    if (!localStorage.getItem(key)) { sendNotification("ChoreQuest ⭐", "Daily summary is ready."); localStorage.setItem(key,"1"); showDailySummary(); }
  }
}
...
function startKidListeners(familyId) {
  ...
  state.unsubKids = onSnapshot(collection(db, "families", familyId, "kids"), snap => {
    ...
    renderLeaderboard("kid-leaderboard", state.kids, state.completions); renderKidChoreGrid(); refreshKidHeader(); refreshKidAvatarPanel(); renderStore();
  });
  state.unsubChores = onSnapshot(collection(db, "families", familyId, "chores"), snap => {
    state.chores = snap.docs.map(d => ({ id:d.id, ...d.data() })); renderKidChoreGrid();
  });
  state.unsubCompletions = onSnapshot(collection(db, "families", familyId, "completions"), snap => {
    state.completions = snap.docs.map(d => ({ id:d.id, ...d.data() }));
...
async function goToParentDashboard(familyId) {
  ...
  renderInvite(); setupParentSettings(); startParentListeners(familyId);
  showScreen("screen-parent-dashboard"); startReminderScheduler();
  // Seed store items for this family
  await seedStoreIfNeeded(db, familyId);
  setTimeout(() => { const h=new Date().getHours(); const key=`parentSum-${todayStr()}`; if(h>=21 && !localStorage.getItem(key)) { localStorage.setItem(key,"1"); showDailySummary(); } }, 1200);
}

async function goToKidDashboard(kid, familyId) {
  state.kidId = kid.id; state.kidDoc = kid; state.familyIdKid = familyId;
...
async function handleParentAuth(isSignUp) {
  ...
  try { if(isSignUp) await createUserWithEmailAndPassword(auth,email,password); else await signInWithEmailAndPassword(auth,email,password); } catch(e) { errEl.textContent = e.message; }
}
async function handleGoogleAuth() { try { await signInWithPopup(auth, new GoogleAuthProvider()); } catch(e) { document.getElementById("auth-error").textContent = e.message; } }
...
async function createFamily() {
  ...
  const errEl = document.getElementById("family-error"); errEl.textContent = "";
...
async function joinFamily() {
  ...
  try {
    ...
    const famDoc = snap.docs[0]; state.familyIdKid = famDoc.id;
    ...
    renderKidSelectGrid(); showScreen("screen-kid-select");
  } catch(e) { errEl.textContent = e.message; }
}
function renderKidSelectGrid() {
  const grid = document.getElementById("kid-select-grid"); if (!grid) return; grid.innerHTML = "";
  if (!state.kids.length) { grid.innerHTML = `<p style="color:var(--text-muted);grid-column:1/-1;text-align:center;">No kids in this family yet.<br/>Ask your parent to add you!</p>`; return; }
  state.kids.forEach(kid => {
    const card = document.createElement("button"); card.className = "kid-select-card";
...
function selectKid(kid) {
  state.kidDoc = kid; state.kidId = kid.id;
  // Check if character selection needed
  if (!kid.baseCharacterId) {
    renderCharSelectGrid(); showScreen("screen-char-select"); return;
  }
  showPinScreen(kid);
}
function showPinScreen(kid) {
  ...
  resetPinDots(); showScreen("screen-kid-pin");
}
...
// ── PIN logic ────────────────────────────────────────────
let pinBuffer = "";
function resetPinDots() { pinBuffer = ""; document.querySelectorAll(".pin-dot").forEach(d => d.classList.remove("filled")); }
...
async function submitPin() {
  ...
  const errEl = document.getElementById("pin-error"); errEl.textContent = "";
  if (state.kidDoc.pin) {
    ...
    try { await updateDoc(doc(db,"families",state.familyIdKid,"kids",state.kidDoc.id), { pin:pinBuffer }); state.kidDoc.pin=pinBuffer; goToKidDashboard(state.kidDoc, state.familyIdKid); }
    catch(e) { errEl.textContent = e.message; }
  }
}
...
function openKidCustomize() {
  ...
  renderColorPicker(); openModal("modal-kid-customize");
}
function renderColorPicker() {
  const el = document.getElementById("kid-color-picker"); if (!el) return; el.innerHTML = "";
  THEME_COLORS.forEach(c => {
    ...
    btn.addEventListener("click", () => { state.selectedThemeColor=c.hex; el.querySelectorAll(".color-swatch").forEach(s=>s.classList.remove("selected")); btn.classList.add("selected"); });
    el.appendChild(btn);
  });
}
async function saveKidCustomize() {
  ...
  try {
    ...
    state.kidDoc.nickname = nickname||state.kidDoc.name; state.kidDoc.themeColor = state.selectedThemeColor;
    applyKidTheme(state.selectedThemeColor); refreshKidHeader(); closeModal("modal-kid-customize");
  } catch(e) { console.error(e); }
}

// ── Logout ───────────────────────────────────────────────
async function parentLogout() { stopListeners(); state.parentUser=null; state.familyId=null; state.familyDoc=null; state.kids=[]; state.chores=[]; state.completions=[]; await signOut(auth); showScreen("screen-landing"); }
function kidLogout() { stopListeners(); state.kidId=null; state.kidDoc=null; state.familyIdKid=null; state.kids=[]; state.chores=[]; state.completions=[]; state.inventory=[]; localStorage.removeItem("cq_kid_session"); showScreen("screen-landing"); }
async function tryRestoreKidSession() {
  ...
  try {
    ...
    await goToKidDashboard(kid, familyId); return true;
  } catch { localStorage.removeItem("cq_kid_session"); return false; }
}
function checkJoinParam() { const p=new URLSearchParams(location.search); const j=p.get("join"); if(j){showScreen("screen-kid-join"); document.getElementById("join-code-input").value=j.toUpperCase(); history.replaceState({},"",location.pathname);} }
function setupParentSettings() {
  const phone = state.familyDoc?.parentPhone||""; const el=document.getElementById("parent-phone-input"); if(el) el.value=phone;
  const nb = document.getElementById("btn-enable-notifs");
  if (nb) { if(Notification.permission==="granted") nb.textContent="✅ Notifications On"; nb.addEventListener("click", async()=>{const ok=await requestNotificationPermission(); nb.textContent=ok?"✅ Notifications On":"Blocked";}); }
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("tab-signin").addEventListener("click", () => { isSignUp=false; document.getElementById("tab-signin").classList.add("active"); document.getElementById("tab-signup").classList.remove("active"); document.getElementById("btn-email-auth").textContent="Sign In"; document.getElementById("signup-name-group").style.display="none"; });
  document.getElementById("tab-signup").addEventListener("click", () => { isSignUp=true; document.getElementById("tab-signup").classList.add("active"); document.getElementById("tab-signin").classList.remove("active"); document.getElementById("btn-email-auth").textContent="Create Account"; document.getElementById("signup-name-group").style.display="flex"; });
  ...
  document.querySelectorAll(".nav-btn").forEach(btn => { btn.addEventListener("click", () => { const p=btn.dataset.panel; document.querySelectorAll(".tab-panel").forEach(p2=>p2.classList.remove("active-panel")); document.getElementById(p).classList.add("active-panel"); document.querySelectorAll(".nav-btn").forEach(b=>b.classList.remove("active")); btn.classList.add("active"); }); });
  ...
  // Manage
  document.getElementById("btn-add-kid").addEventListener("click", () => { document.getElementById("kid-name-input").value=""; document.getElementById("kid-age-input").value=""; document.getElementById("add-kid-error").textContent=""; state.selectedKidEmoji="⭐"; renderEmojiGrid("kid-emoji-picker",KID_EMOJIS,e=>{state.selectedKidEmoji=e;},"⭐"); openModal("modal-add-kid"); });
  document.getElementById("btn-add-chore").addEventListener("click", openAddChore);
  // Invite
  document.getElementById("btn-copy-code").addEventListener("click", () => { navigator.clipboard.writeText(state.familyDoc?.inviteCode||"").then(()=>{document.getElementById("copy-success").textContent="Copied!"; setTimeout(()=>{document.getElementById("copy-success").textContent="";},2000);}); });
  document.getElementById("btn-copy-link").addEventListener("click", () => { const link=`${location.origin}${location.pathname}?join=${state.familyDoc?.inviteCode||""}`; navigator.clipboard.writeText(link).then(()=>{document.getElementById("copy-success").textContent="Link copied!"; setTimeout(()=>{document.getElementById("copy-success").textContent="";},2000);}); });
  ...
  document.querySelectorAll(".num-btn[data-num]").forEach(btn => { btn.addEventListener("click", () => { if(pinBuffer.length>=4)return; pinBuffer+=btn.dataset.num; updatePinDots(); if(pinBuffer.length===4) submitPin(); }); });
  ...
  // Kid nav tabs (Home / Avatar / Store)
  document.querySelectorAll(".kid-nav-tab").forEach(btn => { btn.addEventListener("click", () => {
  ...
  // Store categories
  document.querySelectorAll(".store-cat-btn").forEach(btn => { btn.addEventListener("click", () => { state.storeCategory=btn.dataset.cat; document.querySelectorAll(".store-cat-btn").forEach(b=>b.classList.remove("active")); btn.classList.add("active"); renderStore(); }); });
  // Modals close
  ...
  document.getElementById("btn-save-parent-phone")?.addEventListener("click", async () => {
    ...
    try { await updateDoc(doc(db,"families",state.familyId),{parentPhone:phone}); if(state.familyDoc) state.familyDoc.parentPhone=phone; const msg=document.getElementById("parent-phone-msg"); if(msg){msg.textContent="Saved!"; setTimeout(()=>{msg.textContent="";},2000);} } catch(e){ console.error(e); }
  });
  // Kid notifs
  const nkb=document.getElementById("btn-enable-kid-notifs");
  if(nkb){if(Notification.permission==="granted") nkb.textContent="✅ Notifications On"; nkb.addEventListener("click",async()=>{const ok=await requestNotificationPermission(); nkb.textContent=ok?"✅ Notifications On":"Blocked";});}
  // Close modals via overlay
  document.querySelectorAll(".modal-overlay").forEach(o => { o.addEventListener("click", e => { if(e.target===o) o.classList.add("hidden"); }); });
  // Firebase Auth
  onAuthStateChanged(auth, async user => {
    if (user) { state.parentUser=user; await findOrPromptFamily(user); }
    else {
